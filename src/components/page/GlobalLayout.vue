 ...
 * @LastEditTime: 2019-08-07 14:02:56
 -->
<template>
  <a-layout class="layout" :class="[device]">
    ...
    <a-layout :class="[layoutMode, `content-width-${contentWidth}`]" :style="{ paddingLeft: contentPaddingLeft, minHeight: '100vh', overflowY:'hidden' }">
      ...
      <choose-store v-model="storeVis" :code="$store.getters.code" @onChange="onChange"/>
    </a-layout>
  </a-layout>
</template>
...
export default {
    ...
    watch: {
        ...
        '$store.getters.chooseStore': function (val) {
            this.storeVis = val
        }
    },
    created () {
    ...
    methods: {
        ...mapActions(['setSidebar', 'setChooseStore']),
        onChange (e) {
            if (!e) {
                this.setChooseStore(false)
            } else {
                setTimeout(() => {
