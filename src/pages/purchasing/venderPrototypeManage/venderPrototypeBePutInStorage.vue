      ...
      <a-form :form="form">
        ...
        <a-row>
          ...
          <a-col :sm="16" :md="20" :xl="21">
            <a-form-item>
              <a-textarea placeholder="请输入备注" v-decorator="$check('bill_sample_entryA.note',{initialValue:billsForm.note})" :autosize="{ minRows: 1, maxRows: 2 }" />
            </a-form-item>
          </a-col>
        </a-row>
...
import Preview from './preview/venderPrototypeBePutInStoragePreview'
...
export default {
    ...
    data () {
        return {
            ...
            MenuOperations: [
                {
                    name: '编辑',
                    action: 'venderPrototypeBePutInStorage.edit',
                    fc: row => {
                ...
                {
                    name: '删除',
                    action: 'venderPrototypeBePutInStorage.delete',
                    fc: row => {
                        this.getRow(row, 'del')
                    }
                },
                {
                    name: '确认',
                    action: 'venderPrototypeBePutInStorage.confirm',
                    fc: row => {
                ...
                {
                    name: '取消确认',
                    action: 'venderPrototypeBePutInStorage.cancel',
                    fc: row => {
                ...
                {
                    name: '预览',
                    action: 'venderPrototypeBePutInStorage.get',
                    fc: row => {
    ...
    mounted () {
        this.$nextTick(() => {
            this.tableCode = '64882d7197b8432390f4384e3f799824:venderPrototypeBePutInStorage'
            this.tableApiPath = 'SampleSampleEntryFindEntryPurchase'
    ...
    methods: {
        ...
        submitData (e) {
            ...
            validateFields((err, values) => {
                if (!err) {
                    ...
                    this.loading = true
                    axios({
                        path: this.bt_Save ? 'SampleSampleEntryUpdateEntryPurchase' : 'SampleSampleEntryAddEntryPurchase',
                        method: 'post',
                        body: formDate
                    }).then(res => {
                        this.loading = false
        ...
        onSeach (e) {
            console.log(e, '搜索框数据')
            e.path = 'SampleSampleEntryFindEntryPurchase'
        ...
        onSeachClick (e) {
            if (e === 'add') {
                ...
                this.$refs.newTable.exportFile('SampleSampleEntryFindEntryPurchase', this.seachValue)
                console.log('导出')
            } else if (e === 'print') {
                console.log('打印')
            }
        ...
        onDel (e) {
            ...
            this.$confirm({
                ...
                onOk () {
                    axios({
                        path: 'SampleSampleEntryDeleteEntryPurchase',
                        method: 'post',
                        body: { ids: arr }
                    }).then(res => {
                        if (res.flag === 1) {
    ...
    watch: {
        '$store.stare.app.pageName' (val) {
            if (val === 'venderPrototypeBePutInStorage') {
                // this.getAll()
            }
        },
        clientVisible (val) {
