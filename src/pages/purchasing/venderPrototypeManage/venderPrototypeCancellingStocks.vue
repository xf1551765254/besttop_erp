 ...
 * @Company: BestTop
 * @Author: shuai
 * @Date: 2019-07-16 09:54:23
 * @LastEditors: shuai
 * @LastEditTime: 2019-08-23 18:42:22
 -->
<template>
  <div>
    <!-- 工具栏 -->
      ...
      <a-form :form="form">
        ...
        <a-row>
          ...
          <a-col :sm="16" :md="20" :xl="21">
            <a-form-item>
              <a-textarea placeholder="请输入备注" v-decorator="$check('bill_sample_entry.note',{initialValue:billsForm.note})" :autosize="{ minRows: 1, maxRows: 2 }" />
            </a-form-item>
          </a-col>
        </a-row>
...
<script>
import Goods from './contractGoods/goods'
import Preview from './preview/venderPrototypeCancellingStocksPreview'
...
const columns = [
    ...
    { title: '申请退库数量', dataIndex: 'recN', width: 100, align: 'right', scopedSlots: { customRender: 'recN' } },
...
export default {
    ...
    data () {
        return {
            ...
            MenuOperations: [
                {
                    name: '编辑',
                    action: 'venderPrototypeCancellingStocks.edit',
                    fc: row => {
                ...
                {
                    name: '删除',
                    action: 'venderPrototypeCancellingStocks.delete',
                    fc: row => {
                        this.getRow(row, 'del')
                    }
                },
                {
                    name: '确认',
                    action: 'venderPrototypeCancellingStocks.confirm',
                    fc: row => {
                ...
                {
                    name: '取消确认',
                    action: 'venderPrototypeCancellingStocks.cancel',
                    fc: this.cancelTheConfirmation
                },
                {
                    name: '预览',
                    action: 'venderPrototypeCancellingStocks.get',
                    fc: row => {
    ...
    mounted () {
        this.$nextTick(() => {
            this.tableCode = '2e31e8c4ed1c47d5a0032eb752a5a24e:venderPrototypeCancellingStocks'
            this.tableApiPath = 'SampleSampleExitFindExitPurchase'
    ...
    methods: {
         ...
         * @return:
         */
        handleSearchStoreCode (value) {
            console.log(value)
            if (!value) {
                return
            }
            const path = 'OrganizationStoreFindStoreAndDirect'
            clearTimeout(this.seachTimer)
            this.seachTimer = setTimeout(() => {
                this.distributionCenterArr.length = 0
                this.getData({
        ...
        getData (pams = {}, path) {
            ...
            axios({
                ...
                if (res.flag === 1) {
                    ...
                    } else if (path === 'OrganizationStoreFindStoreAndDirect') {
        ...
        submitData (e) {
            ...
            validateFields((err, values) => {
                if (!err) {
                    ...
                    axios({
                        path: this.bt_Save ? 'SampleSampleExitUpdateExitPurchase' : 'SampleSampleExitAddExitPurchase',
                        method: 'post',
                        body: formDate
                    }).then(res => {
                        this.loading = false
        ...
        onSeach (e) {
            ...
            e.path = 'SampleSampleExitFindExitPurchase'
        ...
        onSeachClick (e) {
            if (e === 'add') {
                ...
                this.$refs.newTable.exportFile('SampleSampleExitFindExitPurchase', this.seachValue)
                console.log('导出')
            } else if (e === 'print') {
                console.log('打印')
            }
        ...
        onDel (e) {
            ...
            this.$confirm({
                ...
                onOk () {
                    axios({
                        path: 'SampleSampleExitDeleteExitPurchase',
                        method: 'post',
                        body: { ids: arr }
                    }).then(res => {
                        if (res.flag === 1) {
    ...
    watch: {
        '$store.stare.app.pageName' (val) {
            if (val === 'venderPrototypeCancellingStocks') {
                // this.getAll()
            }
        },
        clientVisible (val) {
