 ...
 * @LastEditTime: 2019-08-23 17:54:26
 -->
<template>
  <div>
    <h4 class="baseInfo">提货台阶</h4>
    <a-row>
    ...
    <a-table
      ...
      :scroll="scrollXY"
    >
      <template v-for="col in ['priceA','strategyNumA','strategyPriceA','strategyNumB','strategyPriceB','strategyNumC','strategyPriceC','strategyNumD','strategyPriceD','note']" :slot="col" slot-scope="text, record">
        <div :key="col">
          <a-input-number
            v-if="col === 'priceA'"
          ...
          <a-input-number
            ...
            @change="e => InputChange(e, record.tbWebId, col)" />
          <a-input-number
            v-else-if="col === 'strategyNumB'"
            style="margin: -5px 0;width:100%;"
            :min="record.strategyNumA+1"
            :max="Number.MAX_SAFE_INTEGER"
            :value="text"
            :precision="0"
            @change="e => InputChange(e, record.tbWebId, col)" />
          <a-input-number
            v-else-if="col === 'strategyPriceB'"
            style="margin: -5px 0;width:100%;"
            :min="0"
            :max="Number.MAX_SAFE_INTEGER"
            :value="text"
            ...
            @change="e => InputChange(e, record.tbWebId, col)" />
          <a-input-number
            v-else-if="col === 'strategyNumC'"
            style="margin: -5px 0;width:100%;"
            :min="record.strategyNumB+1"
            :max="Number.MAX_SAFE_INTEGER"
            :value="text"
            :precision="0"
            @change="e => InputChange(e, record.tbWebId, col)" />
          <a-input-number
            v-else-if="col === 'strategyPriceC'"
            style="margin: -5px 0;width:100%;"
            :min="0"
            :max="Number.MAX_SAFE_INTEGER"
            :value="text"
            ...
            @change="e => InputChange(e, record.tbWebId, col)" />
          <a-input-number
            v-else-if="col === 'strategyNumD'"
            style="margin: -5px 0;width:100%;"
            :min="record.strategyNumC+1"
            :max="Number.MAX_SAFE_INTEGER"
            :value="text"
            :precision="0"
            @change="e => InputChange(e, record.tbWebId, col)" />
          <a-input-number
            v-else-if="col === 'strategyPriceD'"
            style="margin: -5px 0;width:100%;"
            :min="0"
            :max="Number.MAX_SAFE_INTEGER"
            :value="text"
    ...
    data () {
        return {
            ...
            categoryinfoColumns: [ // 表头
                ...
                { title: '分销价', dataIndex: 'priceA', width: 150, scopedSlots: { customRender: 'priceA' } },
                { title: '台阶1',
                    children: [
                        {
                            title: '提货数', dataIndex: 'strategyNumA', width: 100, scopedSlots: { customRender: 'strategyNumA' }
                        },
                        {
                            title: '政策价', dataIndex: 'strategyPriceA', width: 150, scopedSlots: { customRender: 'strategyPriceA' }
                        }
                    ] },
                { title: '台阶2',
                    children: [
                        {
                            title: '提货数', dataIndex: 'strategyNumB', width: 100, scopedSlots: { customRender: 'strategyNumB' }
                        },
                        {
                            title: '政策价', dataIndex: 'strategyPriceB', width: 150, scopedSlots: { customRender: 'strategyPriceB' }
                        }
                    ] },
                { title: '台阶3',
                    children: [
                        {
                            title: '提货数', dataIndex: 'strategyNumC', width: 100, scopedSlots: { customRender: 'strategyNumC' }
                        },
                        {
                            title: '政策价', dataIndex: 'strategyPriceC', width: 150, scopedSlots: { customRender: 'strategyPriceC' }
                        }
                    ] },
                { title: '台阶4',
                    children: [
                        {
                            title: '提货数', dataIndex: 'strategyNumD', width: 100, scopedSlots: { customRender: 'strategyNumD' }
                        },
                        {
                            title: '政策价', dataIndex: 'strategyPriceD', width: 150, scopedSlots: { customRender: 'strategyPriceD' }
                        }
                    ] },
