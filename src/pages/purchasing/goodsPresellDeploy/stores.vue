 ...
 * @LastEditTime: 2019-08-20 17:32:13
 -->
<template>
  <div>
    <a-modal
...
export default {
    ...
    data () {
        return {
            ...
            columns: [
                ...
                { title: '隶属公司', dataIndex: 'upStoreName' },
    ...
    methods: {
        ...
        //             content: '是否清空当前列表?',
        //             onOk () {
        //                 const selectDatas = that.dataSource.filter(o => is.inArray(o.code, that.selectedRowKeys))
        //                 if (selectDatas.length > 0) {
        //                     selectDatas[0].mun = false
        //                     that.$emit('onSelect', selectDatas)
        //                 }
        //                 that.selectedRowKeys.length = 0
        //                 that.isNotSelect = that.selectedRowKeys.length > 0
        ...
        select (code) {
            ...
            // this.$emit('update:visible', false)
            const that = this
            if (that.$props.objData) {
                that.$confirm({
                    title: '提示',
                    content: '是否清空当前列表?',
                    onOk () {
                        const selectDatas = that.dataSource.filter(o => o.code === code)
                        if (selectDatas.length > 0) {
                            console.log(selectDatas, '单选门店')
                            that.$emit('onSelect', [selectDatas[0]])
                        }
                        that.selectedRowKeys.length = 0
                        that.isNotSelect = that.selectedRowKeys.length > 0
                        that.$emit('update:visible', false)
