 ...
 * @Company: BestTop
 * @Author: lin
 * @Date: 2019-08-06 11:16:20
 * @LastEditors: lin
 * @LastEditTime: 2019-08-22 11:01:41
 -->
<template>
  <div>
    <!-- 工具栏 -->
    ...
    <bt-modal
      ...
      :tags="tags" >
      <a-form :form="form">
        <a-row class="titleSet" style="">促销券使用规则</a-row>
        <a-row>
          ...
          <a-col :sm="16" :md="8" :xl="5">
            <a-form-item>
              <a-input-number
                ...
                :step="1"
                :precision="2"
                v-decorator="$check('CouponRuleVli.standardAmount', {initialValue:tableSingleData.standardAmount||0},standardAmountHandler)"/>
            </a-form-item>
          </a-col>
          ...
          <a-col :sm="16" :md="8" :xl="5">
            <a-form-item>
              <a-input-number
                ...
                :step="1"
                :precision="2"
                v-decorator="$check('CouponRuleVli.reachAmount', {initialValue:tableSingleData.reachAmount||0},standardAmountHandler)"/>
            </a-form-item>
          </a-col>
          <a-col :sm="8" :md="4" :xl="3">用金额</a-col>
          <a-col :sm="16" :md="8" :xl="5">
            <a-form-item>
              <a-input-number
                placeholder="请输入用金额"
                ...
                :step="1"
                :precision="2"
                v-decorator="$check('CouponRuleVli.useAmount', {initialValue:tableSingleData.useAmount||0},useAmountHandler)"/>
            </a-form-item>
          </a-col>
          ...
          <a-col :sm="16" :md="8" :xl="5">
            <a-form-item>
              <a-input-number
                ...
                :step="1"
                :precision="2"
                v-decorator="$check('CouponRuleVli.maxAmount', {initialValue:tableSingleData.maxAmount||0},maxAmountHandler)"/>
            </a-form-item>
          </a-col>
        </a-row>
        <a-row>
      ...
      <div>
        ...
        <a-table
          ...
          <template v-for="col in ['skuName','concatClass','concatBrand','note']" :slot="col" slot-scope="text, record, index">
            <div :key="col">
              <div v-if="col==='skuName'||col==='concatBrand'||col==='concatClass'">
...
import Preview from '@pages/marketing/CouponManagement/preview/CouponUseRulePreview'
export default {
  name: 'CouponUseRule',
  ...
  data () {
    return {
      ...
      menuOpt: [
        { name: '编辑',
          action: 'CouponUseRule.edit',
          fc: this.addEdit
        }, { name: '删除',
          action: 'CouponUseRule.delete',
          fc: this.deleteHandler
        }, { name: '审核',
          action: 'CouponUseRule.verify',
          fc: this.auditHandler
        }, { name: '预览',
          action: 'CouponUseRule.get',
      ...
      tableColumns: [
        ...
        { title: '品类',
          ...
          align: 'center',
          width: 200,
          scopedSlots: { customRender: 'concatClass' } },
        { title: '品牌',
          dataIndex: 'concatBrand',
          align: 'center',
          width: 200,
          scopedSlots: { customRender: 'concatBrand' } },
      ...
      // 下拉框
      effectStoreArr: [],
      couponTypeDropArr: [] // 电子币
    }
  },
  computed: {},
  mounted () {
    this.$nextTick(() => {
      this.tableCode = '63c88a423cbb4da392408e05639db1ca:CouponUseRule'
      this.tableApiPath = 'ScheduleCouponRuleDefineFindRuleDefinePageList'
  ...
  methods: {
    /**
         ...
         */
    onSeach (e) {
      e.path = 'ScheduleCouponRuleDefineFindRuleDefinePageList'
    ...
    onSeachClick (e) {
      if (e === 'add') {
        ...
        this.$refs.newTable.exportFile('ScheduleCouponRuleDefineFindRuleDefinePageList')
      }
    },
    /**
         * @name: 更新表格
         ...
         */
    reLoadTableData () {
      this.$refs.newTable.setTableData(this.tableCode, { path: 'ScheduleCouponRuleDefineFindRuleDefinePageList' })
    },
    /**
         * @name: 表格的多选
         * @msg:
    ...
    onContextmenuChange (tableRow) {
      ...
      if (tableRow.status === 'schedule_coupon_rule_define_status:1') {
        this.disContextMenu = []
      } else {
    ...
    deleteHandler (delEventData) {
      ...
      if (delEventData === -1) {
        ...
        const isAuit = newData.filter(o => o.status !== 'schedule_coupon_rule_define_status:1')
        if (isAuit.length > 0) {
      ...
      this.$confirm({
        ...
        onOk () {
          axios({
            path: 'ScheduleCouponRuleDefineBatchDelete', // 接口path名，必传
            method: 'post',
            body: {
              codeList: arr,
    ...
    auditHandler (autiData) {
      ...
      this.$confirm({
        ...
        onOk () {
          axios({
            path: 'ScheduleCouponRuleDefineBatchVerify', // 接口path名，必传
            method: 'post',
            body: {
    ...
    addEdit (addEditData) {
      if (addEditData === -1) {
        this.isAdd = true
        this.AddEditModalTitle = '新增促销券使用规则'
        ...
        this.isAdd = false
        this.AddEditModalTitle = '编辑促销券使用规则'
    ...
    handleSubmit (e) {
      ...
      validateFields((err, values) => {
        if (!err) {
          ...
          newData.forEach(o => {
            if (!o.skuCode) {
              o.classCodeName = o.concatClass
              o.brandCodeName = o.concatBrand
            }
          })
          ...
          axios({
            path: this.isAdd ? 'ScheduleCouponRuleDefineAddSelective' : 'ScheduleCouponRuleDefineUpdateSelective',
            method: 'post',
            body: values
          }).then(res => {
    ...
    maxAmountHandler (rule, value, callback) {
      const form = this.form
      const useAmount = form.getFieldValue('useAmount')
      if (value && useAmount) {
        form.validateFields(['useAmount'], { force: true })
      }
      if (value === 0) {
        const str = '输入的金额不能为0'
        callback(str)
    ...
    /**
         ...
         * @return:
         */
    useAmountHandler (rule, value, callback) {
      const form = this.form
      if (value && value > form.getFieldValue('maxAmount')) {
        const str = '用金额不能大于上限金额'
        callback(str)
      } else if (value === 0) {
        const str = '输入的金额不能为0'
        callback(str)
    ...
    getStatusDropArr () {
      axios({
        path: 'DistOriginDataQueryScheduleCouponRuleDefineStatusList',
        method: 'post'
      }).then(res => {
        if (res.flag === 1) {
