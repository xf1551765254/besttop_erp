 ...
 * @LastEditors: haile
 * @LastEditTime: 2019-08-26 16:50:49
 -->
<template>
  <fullscreen
    ref="fullscreen"
    ...
    <div class="box" @keydown.tab="tabHandler($event)" @keydown.space="spaceHandler($event)" @keydown.up="currentIndex--" @keydown.down="currentIndex++">
      <!-- 顶部 -->
      <div class="header">
      ...
      <div class="main" ref="main">
        <div class="left">
          ...
          <div class="pay-methods">
            <a-row>
              <a-col :span="8" v-for="(item, index) in payMethods" :key="index">{{ `${item.description} ${item.keyNote}` }}</a-col>
            </a-row>
          </div>
        </div>
        <div class="center">
    ...
    >
      ...
      <div class="wz-container">
        <a-row>
          ...
          <a-col :span="18">
            <a-input-number :autoFocus="true" v-model="wzPay.wzAmount" :max="amountList.balanceAmount" :disabled="ifDeposit === 1"/>
          </a-col>
...
export default {
    ...
    methods: {
        ...
        async queryOrderInfo (statusList) {
            const res = await axios({
        ...
        getPayMethods () {
            axios({
                ...
                if (res.flag === 1) {
                    const data = res.data
                    data.forEach(item => {
                        item.description = item.description.slice(0, -2)
                        if (item.value === 'wechat_pay') {
                            item.keyNote = 'F1'
                        } else if (item.value === 'alipay') {
                            item.keyNote = 'F2'
                        } else if (item.value === 'xinbai_pay') {
                            item.keyNote = 'F3'
                        } else if (item.value === 'cash_pay') {
                            item.keyNote = 'F4'
                        } else if (item.value === 'credit_pay') {
                            item.keyNote = 'F5'
                        } else if (item.value === 'valuecard_pay') {
                            item.keyNote = 'F6'
                        }
                    })
