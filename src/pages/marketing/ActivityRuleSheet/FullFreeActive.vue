 ...
 * @Company: BestTop
 * @Author: lin
 * @Date: 2019-07-18 11:09:12
 * @LastEditors: lin
 * @LastEditTime: 2019-08-28 17:16:53
 -->
<template>
  <div>
    <!-- 工具栏 -->
    ...
    <bt-modal
      ...
      <!-- 满免规则 -->
      <div>
        <a-row class="titleSet" style="margin-top:10px">满免规则商品列表</a-row>
        <a-button size="small" @click="addFreeData" style="margin-bottom:5px">添加满免规则</a-button>
        <a-table
          rowKey="tb_web_id"
          ...
          bordered
          :pagination="false"
          :dataSource="detailDataFree" >
          <template v-for="col in ['note','type','rateOne','exemptionRateOne']" :slot="col" slot-scope="text, record, index">
            <div :key="col">
              <a-input-number
                v-if="col === 'rateOne'"
                ...
                :precision="2"
                :value="text"
                @change="e => InputFreetDataChange(e, record.tb_web_id, col)" />
              <a-input-number
                v-if="col === 'exemptionRateOne'"
                style="margin: -5px 0;width:100%;"
                :min="0.00"
                :max="100"
                :step="1"
              ...
              <a-input
                ...
                style="margin: -5px 0; width:100%"
                :value="text"
                @change="e => InputFreetDataChange(e.target.value, record.tb_web_id, col)"/>
            </div>
          </template>
          <template slot="action" slot-scope="text, record, index">
            <div class="editable-row-operations">
              <span>
                <a-popconfirm title="是否确定删除?" @confirm="() => deleteFreeDataRow(record.tb_web_id)">
                  <a>删除</a>
                </a-popconfirm>
              </span>
            </div>
...
import Preview from '@pages/marketing/ActivityRuleSheet/preview/FullFreeActivePreview'
...
export default {
    ...
    data () {
        return {
            ...
            menuOpt: [
                { name: '编辑',
                    action: 'FullFreeActive.edit',
                    fc: this.addEdit
                }, { name: '删除',
                    action: 'FullFreeActive.delete',
                    fc: this.deleteHandler
                }, { name: '审核',
                    action: 'FullFreeActive.verify',
            ...
            tableColumnsFree: [
                ...
                { title: '满免方式', dataIndex: 'type', align: 'center', scopedSlots: { customRender: 'type' } },
                { title: '分摊比率（%）',
                    dataIndex: 'rateOne',
                    align: 'center',
                    scopedSlots: { customRender: 'rateOne' } },
                { title: '满免占比（%）',
                    dataIndex: 'exemptionRateOne',
                    align: 'center',
                    scopedSlots: { customRender: 'exemptionRateOne' } },
                { title: '备注', dataIndex: 'note', align: 'center', scopedSlots: { customRender: 'note' } },
                { title: '操作', width: 70, key: 'operation', align: 'center', scopedSlots: { customRender: 'action' } }
            ],
            detailDataFree: [],
    ...
    mounted () {
        this.seachValue.scheduleType = 'schedule_rule_define_schedule_type:7'
        this.$nextTick(() => {
            this.tableCode = 'c0148116335b4919825152ee32f45285:FullFreeActive'
            this.tableApiPath = 'ScheduRuleQueryScheduleRule'
            this.$refs.newTable.setContextmenu(this.menuOpt)
        })
        this.getEffectStoreArr()
        this.getScheduleDefineIdDorpArr()
        this.getFreetTypeDorpArr()
    },
    methods: {
        // 主表格搜索
        /**
        ...
        addEdit (addEditData) {
            if (addEditData === -1) {
                this.isAdd = true
                this.AddEditModalTitle = '新增满免活动规则单'
                ...
                const { mul } = this.$math
                this.isAdd = false
                this.AddEditModalTitle = '编辑满免活动规则单'
                ...
                axios({
                    ...
                    if (res.flag === 1) {
                        ...
                        const dataTow = [...res.data.exemptionList]
                        data.forEach(item => {
                            item.tb_web_id = item.id
                        })
                        dataTow.forEach(item => {
                            item.tb_web_id = item.id
                            item.rateOne = mul(item.rate, 100, 2)
                            item.exemptionRateOne = mul(item.exemptionRate, 100, 2)
                        })
                        this.detailTableData = data
                        this.detailDataFree = dataTow
                    }
                })
            }
        ...
        handleSubmit (e) {
            ...
            validateFields((err, values) => {
                if (!err) {
                    ...
                    const newDataTow = [...this.detailDataFree]
                    if (newData.length === 0) {
                        return this.$message.error('请选择商品明细')
                    }
                    if (newDataTow.length === 0) {
                        return this.$message.error('请新增满免规则')
                    }
                    newDataTow.forEach(item => {
                        item.rate = div(item.rateOne, 100, 4)
                        item.exemptionRate = div(item.exemptionRateOne, 100, 4)
                    })
                    if (!this.isAdd) {
         ...
         */
        addFreeData () {
            if (this.detailDataFree.length >= 1) {
                this.$message.warning('满免规则设置只能设置1条数据')
            } else {
                const obj = {
                    tb_web_id: uuid.v1().replace(/-/g, ''),
                    type: 'schedule_rule_full_exemption_type:2',
                    rateOne: 0,
                    exemptionRateOne: 0,
                    note: ''
                }
                this.detailDataFree.push(obj)
            }
        },
        /**
         * @name: 满免规则编辑
         ...
         * @return:
         */
        InputFreetDataChange (val, keyID, key) {
            const newData = [...this.detailDataFree]
            const target = newData.filter(o => o.tb_web_id === keyID)[0]
            if (target) {
                if (key === 'rateOne' || key === 'exemptionRateOne') {
                    target[key] = val || 0
                } else {
                    target[key] = val
                }
                this.detailDataFree = newData
            }
        },
        /**
         * @name: 满免规则删除
         ...
         * @return:
         */
        deleteFreeDataRow (keyId) {
            const newData = [...this.detailDataFree]
            this.detailDataFree = newData.filter(o => o.tb_web_id !== keyId)
        },
        /**
         * @name:  生效分店
         * @msg:
        ...
        getFreetTypeDorpArr () {
            axios({
                ...
                method: 'post',
                body: {
                    type: 'schedule_rule_full_exemption_type'
                }
            }).then(res => {
                if (res.flag === 1) {
