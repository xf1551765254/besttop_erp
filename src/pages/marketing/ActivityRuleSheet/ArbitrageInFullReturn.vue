 ...
 * @Company: BestTop
 * @Author: lin
 * @Date: 2019-07-18 11:10:12
 * @LastEditors: lin
 * @LastEditTime: 2019-08-28 17:15:30
 -->
<template>
  <div>
    <!-- 工具栏 -->
      ...
      <div>
        ...
        <div>
          <a-table
            ...
            <template v-for="col in ['skuName','concatClass','concatBrand','rateProviderOne','note','reachUnitPrice']" :slot="col" slot-scope="text, record, index">
              <div :key="col">
...
import Preview from '@pages/marketing/ActivityRuleSheet/preview/ArbitrageInFullReturnPreview'
...
export default {
    ...
    data () {
        return {
            ...
            seachValue: {
                scheduleType: 'schedule_rule_define_schedule_type:4',
            ...
            menuOpt: [
                { name: '编辑',
                    action: 'ArbitrageInFullReturn.edit',
                    fc: this.addEdit
                }, { name: '删除',
                    action: 'ArbitrageInFullReturn.delete',
                    fc: this.deleteHandler
                }, { name: '审核',
                    action: 'ArbitrageInFullReturn.verify',
                    ...
                    action: 'ArbitrageInFullReturn.get',
    ...
    mounted () {
        this.seachValue.scheduleType = 'schedule_rule_define_schedule_type:4'
        this.$nextTick(() => {
            this.tableCode = 'acdc641ec03e45fb8d1ff25bf80a38df:ArbitrageInFullReturn'
    ...
    methods: {
        ...
        addEdit (addEditData) {
            if (addEditData === -1) {
                this.isAdd = true
                this.AddEditModalTitle = '新增套购满返活动规则单'
                ...
                const { mul } = this.$math
                this.isAdd = false
                this.AddEditModalTitle = '编辑套购满返活动规则单'
        ...
        handleSubmit (e) {
            ...
            validateFields((err, values) => {
                if (!err) {
                    ...
                    values.scheduleType = 'schedule_rule_define_schedule_type:4'
         ...
         * @return:
         */
        coinReachAmountMaxHandler (rule, value, callback) {
            const form = this.form
            const coinMaxAmount = form.getFieldValue('coinMaxAmount')
            if (value && coinMaxAmount) {
                form.validateFields(['coinMaxAmount'], { force: true })
            }
            if (value === 0) {
                const str = '输入的金额不能为0'
                callback(str)
        ...
        coinMaxAmountHandler (rule, value, callback) {
            ...
            if (value === 0) {
                const str = '输入的金额不能为0'
                callback(str)
            } else if (value && value > form.getFieldValue('coinReachAmount')) {
                const str = '上限金额不能大于满金额'
                callback(str)
            } else {
                callback()
            }
