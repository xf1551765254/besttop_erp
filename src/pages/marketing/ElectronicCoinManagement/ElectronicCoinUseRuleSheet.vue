 ...
 * @Company: BestTop
 * @Author: lin
 * @Date: 2019-07-24 11:07:56
 * @LastEditors: lin
 * @LastEditTime: 2019-08-19 15:05:14
 -->
<template>
  <div>
    <!-- 工具栏 -->
          ...
          <a-col :sm="16" :md="8" :xl="5">
            <a-form-item>
              <a-input-number
                ...
                :step="1"
                :precision="2"
                v-decorator="$check('ElectronicCoinUse.coinStandardAmount', {initialValue:tableSingleData.coinStandardAmount||0},standardAmountHandler)"/>
            </a-form-item>
          </a-col>
          ...
          <a-col :sm="16" :md="8" :xl="5">
            <a-form-item>
              <a-input-number
                ...
                :step="1"
                :precision="2"
                v-decorator="$check('ElectronicCoinUse.coinReachAmount', {initialValue:tableSingleData.coinReachAmount||0},standardAmountHandler)"/>
            </a-form-item>
          </a-col>
          ...
          <a-col :sm="16" :md="8" :xl="5">
            <a-form-item>
              <a-input-number
                ...
                :step="1"
                :precision="2"
                v-decorator="$check('ElectronicCoinUse.coinUseAmount', {initialValue:tableSingleData.coinUseAmount||0},coinUseAmountHandler)"/>
            </a-form-item>
          </a-col>
          ...
          <a-col :sm="16" :md="8" :xl="5">
            <a-form-item>
              <a-input-number
                ...
                :step="1"
                :precision="2"
                v-decorator="$check('ElectronicCoinUse.coinMaxAmount', {initialValue:tableSingleData.coinMaxAmount||0},coinMaxAmountHandler)"/>
            </a-form-item>
          </a-col>
        </a-row>
        <a-row>
...
import Preview from '@pages/marketing/ElectronicCoinManagement/preview/ElectronicCoinUseRuleSheetPreview'
export default {
        ...
        return {
            conditions: [
                ...
                },
                {
                    ...
                    body: {
                        type: 'schedule_coin_rule_define_status'
                    }
                },
                {
                    key: 'startTime',
            ...
            menuOpt: [
                { name: '编辑',
                    action: 'ElectronicCoinUseRuleSheet.edit',
                    fc: this.addEdit
                }, { name: '删除',
                    action: 'ElectronicCoinUseRuleSheet.delete',
                    fc: this.deleteHandler
                }, { name: '审核',
                    action: 'ElectronicCoinUseRuleSheet.verify',
                    fc: this.auditHandler
                }, { name: '预览',
                    action: 'ElectronicCoinUseRuleSheet.get',
    ...
    mounted () {
        this.$nextTick(() => {
            this.tableCode = '7b11ff9549f74ca7b62aff88815c90a6:ElectronicCoinUseRuleSheet'
            this.tableApiPath = 'CoinRuleQueryRuleDefineList'
    ...
    methods: {
         ...
         */
        onSeach (e) {
            e.path = 'CoinRuleQueryRuleDefineList'
        ...
        onSeachClick (e) {
            if (e === 'add') {
                ...
                this.$refs.newTable.exportFile('CoinRuleQueryRuleDefineList')
            }
        },
        /**
         * @name: 更新表格
         ...
         */
        reLoadTableData () {
            this.$refs.newTable.setTableData(this.tableCode, { path: 'CoinRuleQueryRuleDefineList' })
        },
        /**
         * @name: 表格的多选
         * @msg:
        ...
        onContextmenuChange (tableRow) {
            ...
            if (tableRow.status === 'schedule_coin_rule_define_status:1') {
                this.disContextMenu = []
            } else {
        ...
        deleteHandler (delEventData) {
            ...
            if (delEventData === -1) {
                ...
                const isAuit = newData.filter(o => o.status !== 'schedule_coin_rule_define_status:1')
                if (isAuit.length > 0) {
            ...
            this.$confirm({
                ...
                onOk () {
                    axios({
                        path: 'CoinRuleDeleteCoinUseRuleDefine', // 接口path名，必传
                        method: 'post',
                        body: {
                            ids: arr
                        }
        ...
        auditHandler (autiData) {
            ...
            this.$confirm({
                ...
                onOk () {
                    axios({
                        path: 'CoinRuleAuditRuleDefine', // 接口path名，必传
                        method: 'post',
                        body: {
                            ids: [ autiData.id ]
                        }
        ...
        addEdit (addEditData) {
            if (addEditData === -1) {
                this.isAdd = true
                this.AddEditModalTitle = '新增电子币使用规则'
                ...
                this.isAdd = false
                this.AddEditModalTitle = '编辑电子币使用规则'
        ...
        handleSubmit (e) {
            ...
            validateFields((err, values) => {
                if (!err) {
                    ...
                    axios({
                        path: this.isAdd ? 'CoinRuleAddRuleDefine' : 'CoinRuleUpdateRuleDefine',
                        method: 'post',
                        body: values
                    }).then(res => {
         ...
         * @return:
         */
        coinMaxAmountHandler (rule, value, callback) {
            const form = this.form
            const coinUseAmount = form.getFieldValue('coinUseAmount')
            if (value && coinUseAmount) {
                form.validateFields(['coinUseAmount'], { force: true })
            }
            if (value === 0) {
                const str = '输入的金额不能为0'
                callback(str)
         ...
         * @return:
         */
        coinUseAmountHandler (rule, value, callback) {
            const form = this.form
            if (value && value > form.getFieldValue('coinMaxAmount')) {
    ...
    watch: {
        '$store.state.app.pageName' (to, form) {
            if (to === 'ElectronicCoinUseRuleSheet') {
                this.getCoinTypeDropArr()
                this.getEffectStoreArr()
            }
        },
