 ...
 * @LastEditTime: 2019-08-20 11:42:59
 -->
<template>
  <div class="distributionOrder">
    ...
    <a-row :gutter="10" type="flex" justify="center" class="search">
      ...
      <a-col :xxl="8" :xl="12" :md="16" class="operation">
        ...
        <a-col>
          <a-button size="large" @click="onOPenCustomer" style="width:102px">未提交({{ customerCode?skuCount:0 }})</a-button>
        </a-col>
        <a-col>
          <a-button type="primary" size="large" style="width:85px" @click="isChange(2)">订单({{ notSubmittedCount }})</a-button>
        </a-col>
      </a-col>
    </a-row>
    <!-- 筛选栏 -->
            ...
            <a-row type="flex" justify="start">
              ...
              >
                ...
                <a-form-item
                  label="提货类型"
                >
                  <a-select v-model="packupCode" style="width: 160px" @change="packupChange">
    ...
    >
      ...
      >
        <h6 v-if="customerGoods.length<=0">暂无未提交订单</h6>
        <div v-else>
          <div :key="index" v-for="(item,index) in customerGoods" class="customer-goods">
            ...
            <a-checkbox-group @change="onChange(item)" class="checkbox-cart" v-model="checkedObj[item.code]" >
              <a-checkbox :value="rep" class="cart-item" :key="rep.skuCode" v-for="rep in item.wholesaleOrderDetailList">
          ...
          <a-button type="primary" size="large" style="float:right;margin-top:14px;" @click="updateCommitOrde">提交订单</a-button>
        </div>
      </a-drawer>
    </a-drawer>
    <!-- 当前客户购物车抽屉 -->
...
export default {
    ...
    methods: {
        ...
        async handleChange (data) {
            ...
            const list = JSON.parse(JSON.stringify(this.WarehouseList))
            list.forEach(el => {
                if (data === el.code) {
        ...
        onChange (item) {
            ...
            item.indeterminate = !!checkedList.length && (checkedList.length < item.wholesaleOrderDetailList.length)
            item.checkAll = checkedList.length === item.wholesaleOrderDetailList.length
        ...
        queryOrderCollectionByparam (id) {
            const code = { status: 0, saleStoreCode: this.storeInfo.code, customerCode: id || this.customerCode, createBy: this.userInfo.code }
            axios({
                path: 'DistWholesaleOrderQueryOrderCollectionByparam',
                method: 'post',
                body: code
            }).then(res => {
                if (res.flag === 1) {
                    ...
                    if (res.data && Array.isArray(res.data)) {
                        ...
                        data.forEach(el => {
                            ...
                            el.wholesaleGoodList.forEach(item => {
                                if (item.wholesaleOrderDetailList) {
                                    item.wholesaleOrderDetailList.forEach(sku => {
        ...
        QueryNotSubmitCustomerList () {
            const code = { status: 0, takeStoreCode: this.warehouse.code, customerCode: this.customerCode, createBy: this.userInfo.code }
            axios({
                path: 'DistWholesaleOrderQueryNotSubmitCustomerList',
                method: 'post',
                body: code
            }).then(res => {
                if (res.flag === 1) {
        ...
        onCheckAllChange (item, e) {
            ...
            const codelist = []
            if (item && Array.isArray(item.wholesaleOrderDetailList)) {
                item.wholesaleOrderDetailList.forEach(el => {
                    codelist.push(el)
                })
            }
            ...
            if (this.totalNUm > 0) {
                ...
                data.forEach(el => {
                    ...
                    if (keys.includes(el.code) && this.checkedObj[el.code].length > 0) {
                        ...
                        el.wholesaleOrderDetailList = this.checkedObj[el.code]
                ...
                list.forEach(el => {
                    // console.log(el)
                    el.wholesaleOrderDetailList.forEach(i => {
                        // console.log(i)
                        if (i.goodPic) {
                            delete i.goodPic
                        }
